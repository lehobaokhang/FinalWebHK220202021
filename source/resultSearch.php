...
<html lang="en">
    ...
    <head>
        ...
        <title>Các ứng dụng dành cho bạn trên meApp</title>
    ...
    ?>
        ...
        <main class="pb-4">
            ...
            <section class="book-genre-container mx-auto p-1">
                <h4 class="font-style-2 font-size-md color-2  border-bottom my-1 py-1 black-highlight pointer">Kết quả
                    tìm kiếm</h4>
                <section class="book-cards-container d-flex flex-wrap justify-between">
                    <?php
                $result = search_result($_GET['search_info']);
                if (empty($result))
                    echo '<p class="font-style-2 font-size-md color-1 pointer black-highlight">Không tìm thấy</p>';
                else {
                    foreach ($result as $info) {
                        if ($info[8] === 0) $temp = 'Miễn phí';
                        else $temp = $info[8] . ' VNĐ';
                        $image_path = './uploads/' . $info[0] . '/icon_' . $info[1] . '.jpg';
                        ...
                        echo '<p class="font-style-2 font-size-md color-1 pointer black-highlight">' . $info[1] . '</p>';
                        echo '<div>';
                        echo '<p class="font-size-sm font-style-2 color-3 gray-highlight"> ' . $info[3] . '</p>';
                        echo '<p class="font-size-s font-style-2 color-3  py-1 gray-highlight">Rating: ' . $info[4] . '/5</p>';
                        ...
                        // echo '<a href="#" class="default-btn font-size-sm font-style-2 bg-color-1 mr-half my-half">Cài đặt</span></a>';
                        echo '<a href="info_app.php?app_id=' . $info[0] . '" class="default-btn font-size-sm font-style-2 bg-color-1 mr-half my-half">Xem</span></a>';
                        echo '</p> ';
                        echo '</div>';
                        echo '</div> ';
                        echo '</article>';
